...
<!-- This chunk inspects the input but hides the code and output -->
...
<!-- Read data for all studies and prepare for analyses -->
...
# mean(S2.full.coef_table$ess_bulk)
# min(S2.full.coef_table$ess_bulk)
# max(S2.full.coef_table$ess_bulk)
# 
# mean(S2.null.coef_table$ess_bulk)
# min(S2.null.coef_table$ess_bulk)
# max(S2.null.coef_table$ess_bulk)
...
<!-- weitere zahlen -->
```{r summaryresults,  echo=FALSE, eval=F, warning=F, message=F}
...
For a side-by-side comparison of the developmental trajectories for all conditions in study 2, see figure \@ref(fig:plot-study2). For an additional conventional analysis binning participants according to their age in years, please see table \@ref(tab:suppl-descriptives-S2) in appendix C.

```{r plot-study2, fig.cap = "*Developmental Trajectories for all Conditions in Study 2.* Panels illustrate an example stimulus combination (distractor, cue, target) and results for the four conditions. Coloured lines indicate smoothed mean performance by age. Shaded areas represent 95% credibility intervals. The dashed line demarcates chance level and the dots represent individual means. The coloroured dots and annotation indicate when children's performance exceeds chance level in the respective conditions.", fig.align = "center", echo = FALSE, out.width = "100%"}
...
For a side-by-side comparison of the developmental trajectories for all conditions in study 2, see figure \@ref(fig:plot-study3). For an additional conventional analysis binning participants according to their age in years, please see table \@ref(tab:suppl-descriptives-S3) in appendix C.

```{r plot-study3, fig.cap = "*Developmental Trajectories for all Conditions in Study 3.* Panels illustrate an example stimulus combination (distractor, cue, target) and results for the four conditions. Coloured lines indicate smoothed mean performance by age. Shaded areas represent 95% credibility intervals. The dashed line demarcates chance level and the dots represent individual means. The coloroured dots and annotation indicate when children's performance exceeds chance level in the respective conditions.", fig.align = "center", echo = FALSE, out.width = "100%"}
